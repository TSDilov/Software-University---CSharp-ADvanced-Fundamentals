...
namespace LongerLine
{
    class Program
    {
        ...
        static void ClosesToCenter(double a, double b, double c, double d)
        {
            double distance1 = Math.Sqrt(Math.Pow(Math.Abs(a), 2) + Math.Pow(Math.Abs(b), 2));
            double distance2 = Math.Sqrt(Math.Pow(Math.Abs(c), 2) + Math.Pow(Math.Abs(d), 2)); ;
        ...
        static void LongestLine(double a, double b, double c, double d, double e, double f, double g, double h) 
        {
            ...
            {
                ClosesToCenter(a, b, c, d);
            }
            else
            {
                ClosesToCenter(e, f, g, h);
            }
        }
